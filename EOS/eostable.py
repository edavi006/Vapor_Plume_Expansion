...
class extGADtable:
    ...
    def readStdGadget(self, fname):
        ...
        self.U = []
        self.cs = []
        with open(fname, "r") as stdGadg:
            for i, n in enumerate(stdGadg.read().split()):
                if i == 0:
                    self.ND = int(float(n))
                elif i == 1:
                    self.NS = int(float(n))
                elif i < self.ND + 2:
                    self.rho.append(float(n))
                elif i < self.NS + self.ND + 2:
                    self.S.append(float(n))
                elif i < self.NS*self.ND + self.NS + self.ND + 2:
                    self.P.append(float(n))
                elif i < 2*self.NS*self.ND + self.NS + self.ND + 2:
                    self.T.append(float(n))
                elif i < 3*self.NS*self.ND + self.NS + self.ND + 2:
                    self.U.append(float(n))
                else:
                    self.cs.append(float(n))

        self.rho = np.asarray(self.rho)
        self.S = np.asarray(self.S)
        self.P = np.asarray(self.P).reshape((self.NS, self.ND))
        self.T = np.asarray(self.T).reshape((self.NS, self.ND))
        self.U = np.asarray(self.U).reshape((self.NS, self.ND))
        self.cs = np.asarray(self.cs).reshape((self.NS, self.ND))

    def readExtGadget(self, fname):
        with open(fname, "r") as extGadg:
            self.rho = []
            self.S = []
            self.A = []
            self.cv = []
            ...
            with open(fname, "r") as stdGadg:
                for i, n in enumerate(stdGadg.read().split()):
                    if i == 0:
                        ...
                        self.A.append(float(n))
                    elif i < 2*self.NS*self.ND + self.NS + self.ND + 2:
                        self.cv.append(float(n))
                    elif i < 3*self.NS*self.ND + self.NS + self.ND + 2:
                        self.KPA.append(float(n))
                    else:
                        self.MDQ.append(float(n))

        self.rho = np.asarray(self.rho)
        self.S = np.asarray(self.S)
        self.A = np.asarray(self.A).reshape((self.NS, self.ND))
        self.cv = np.asarray(self.cv).reshape((self.NS, self.ND))
        self.KPA = np.asarray(self.KPA).reshape((self.NS, self.ND))
        self.MDQ = np.asarray(self.MDQ).reshape((self.NS, self.ND))
...
#        print('Using the iSALE Tillotson implementation.')
    ...
    def calchugoniot(self, r00=None, e0=None, pmax=None, writefilename=None):
        ...
        print('calchugoniot:',ir0,ie0,r0,p0,e0,up0,us0)
        self.hug.rho = np.append(self.hug.rho, self.rho[ir0])
        self.hug.P = np.append(self.hug.P, p0)
        self.hug.U = np.append(self.hug.U, e0)
        self.hug.up = np.append(self.hug.up, up0)
